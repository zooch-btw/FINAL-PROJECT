    ...
    this.zoom = new Proxy(this.zoom, {
      set(tgt, prop, receiver) {
        ...
        tgt.src.setAttribute(
          "style",
          `top: ${tgt.y}px; left: ${tgt.x}px; background: url(${
            tgt.img
...
class Product {
  ...
  buildCart(destination) {
    ...
    const x = create("div", "", "cart-close");
    x.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-lg" viewBox="0 0 16 16">
  <path d="M2.146 2.854a.5.5 0 1 1 .708-.708L8 7.293l5.146-5.147a.5.5 0 0 1 .708.708L8.707 8l5.147 5.146a.5.5 0 0 1-.708.708L8 8.707l-5.146 5.147a.5.5 0 0 1-.708-.708L7.293 8z"/>
</svg>
`;
    card.append(x);
...
const PRODUCTS = {
  list: [],
  results: [],
  loc: document.querySelector("#cardRow"),
  init() {
    fetch("products.txt")
  ...
  search(arg = null) {
    if (arg != null && arg.length > 0) {
      ...
      this.results = [];
      this.results.push(...fuse.search(arg));
      this.redisplay();
    } else {
      this.loc.innerHTML = "";
      this.list.forEach((e) => e.build(this.loc));
    }
  },
  redisplay() {
    this.loc.innerHTML = "";
    this.results.forEach((e) => e.item.build(this.loc));
  },
};
const CART = {
  list: [],
  ...
  add(param = 0) {
    ...
    this.subtotal = 0;
    this.list.forEach((e) => {
      let price = Number(e.price.substring(1)) * e.number;
      this.subtotal += price;
    });
